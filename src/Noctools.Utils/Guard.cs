...
namespace Noctools.Utils
{
    ...
    public static class Guard
    {
        ...
        /// <exception cref="TException">Thrown when <paramref name="condition" /> is false</exception>
        /// <remarks><see cref="TException" /> must have a constructor that takes a single string</remarks>
        public static void That<TException>(bool condition, string code = "", string message = "", string userFriendlyMessage = "") where TException : Exception
        {
            if (condition)
            {
                throw !string.IsNullOrEmpty(userFriendlyMessage)
                    ? (TException)Activator.CreateInstance(typeof(TException), code, message, userFriendlyMessage)
                    : (TException)Activator.CreateInstance(typeof(TException), code, message);
            }
        }
