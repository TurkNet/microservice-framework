...
namespace Noctools.EntityFramework
{
    public class QueryRepositoryFactory : IQueryRepositoryFactory
    {
        ...
        public IQueryRepositoryWithId<TEntity, TId> QueryRepository<TEntity, TId>()
    ...
    public class QueryRepository<TDbContext, TEntity> : QueryRepositoryWithId<TDbContext, TEntity, Guid>,
        ...
        where TEntity : class, IAggregateRoot
    {
        ...
        public CachedQueryRepository(QueryRepository<TEntity> queryRepository, IMemoryCache cache,
            IConfiguration config)
        {
            ...
            _cacheOptions =
                new MemoryCacheEntryOptions().SetAbsoluteExpiration(
                    relative: TimeSpan.FromSeconds(cacheSection.GetValue("ExpiredTime", 60)));
        }

        public IQueryable<TEntity> Queryable(int page = 0, int pageSize = 10)
        {
